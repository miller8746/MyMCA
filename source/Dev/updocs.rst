...
Note: It is highly recommended that you use a virtual environment while working with this documentation.  More on Python virtual environments |py3 venv link|.

- Once Sphinx is installed, retrieve the latest version of the :code:`docs` branch of MyMCA.
- After the :code:`/source` directory is edited with the desired changes, run :code:`sphinx-build -b html source build` in the directory containing :code:`/source` and :code:`/build`.  More information can be found on Sphinx builds |sphinx build link|.
- Once satisfactory changes have been made, push the new :code:`/source` and :code:`/build` directories to the :code:`docs` branch.
