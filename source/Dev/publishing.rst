...
First, visit the |main branch|.  The :code:`/MyMCA-dist` directory contains three folders, one called :code:`/frontend`, one called :code:`/db`, and one called :code:`/backend`.  The :code:`/frontend` directory contains the Vue.js project, while the :code:`/backend` directory contains the Node.js project.  The :code:`/db` folder contains the script for creating and populating the database, and will only ever need to be changed when database tables are altered.
